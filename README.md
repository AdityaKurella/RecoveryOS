...
RecoveryOS is an autonomous economic decisioning engine that transforms payment failure recovery from blind retries into a portfolio-optimized, value-maximizing system.
...
## 2. Complete V3 System Architecture
...
[Durable SQLite State Store]
     │
     ▼
[Cryptographic Audit Lineage Engine]
```

---

## 3. Benchmark Summary (559 Held-Out Test Cases)
...
> **Key Result**: The V3 Promoted Hybrid Policy achieves **+₹816.63 higher expected net recovery** over V2 Control, closing **99.4% of the gap to the Rules baseline** (within ₹4.99 of the ceiling).
...
4. **₹35 Uncertainty-Aware Fallback Policy**: When the top ML action predicted net margin is $< \text{₹35}$, the system defaults to domain heuristic rules to prevent low-confidence errors.
5. **Bounded Autonomy & Safety Policy Engine**: High-value payments ($\ge \text{₹10,000}$) with low confidence ($< 0.70$) escalate to manual review (`HUMAN`).
...
## 6. Technical Limitations & Disclaimers

- **Synthetic Counterfactual Benchmark**: Evaluated revenue numbers represent simulated expected net recovery on a 559 held-out synthetic test dataset.
...
- **Local SQLite Persistence**: Persistence uses a local SQLite database for process restart resilience rather than a distributed cluster store.
